...
- **简介正式、少技术细节**：简介／一句话定位用正式、通顺的书面语，说清「这是什么、解决什么问题」即可；别堆术语和技术细节，让不太懂技术的人也能看懂。
- **预览图若不是核心卖点，可省**——尤其在不支持截图的环境，别留空图占位。
- **许可证必填**，否则别人不敢用。

### 1.1 简介 / 一句话定位：正式、别堆技术细节

简介和一句话定位是读者第一眼看到的东西：**用正式、通顺的书面语**，说清「这是什么、解决什么问题」；
**不要堆技术细节**，也别写成大白话/口语。让不太懂技术的人也能看懂。

| 别这么写（堆技术细节） | 建议这么写（正式、少细节） |
|---|---|
| 「基于事件驱动架构与 WebSocket 长连接的实时协作引擎」 | 「面向团队的实时协作工具，支持多人同步编辑。」 |
| 「为开发者打造的声明式配置工作流（基于 YAML 与模板引擎）」 | 「通过配置文件即可定义一套自动化流程，上手简单。」 |
| 「一站式解决方案，赋能业务闭环」 | 「把多个步骤整合到一个工具里，一次即可完成。」 |

> 正式≠堆术语。技术细节可以在后文的「特性 / 用法」里展开；简介先把「这是什么、解决什么问题」讲清楚。

### 1.2 描述项目的几条要求（写作口径）

写简介、概述、功能这类介绍项目的文字时，再遵守这几条：

1. **项目导向，别写流程**：介绍项目本身（是什么、解决什么问题、有什么能力），
   不要写成「怎么做出来的 / 一步一步怎么运行」的流程说明。
2. **正式书面，但不堆术语**：用正式、通顺的书面语；能说清就不堆技术名词，
   术语留到后文需要处再展开。
3. **不暴露个人 / 环境细节**：不提个人邮箱、特定目录、本机路径、私有环境、
   具体机器配置等；面向读者，不面向你的开发环境。
4. **不罗列堆砌文件名、路径**：不要为了凑数把一堆文件名、目录路径列出来；
   需要指路时给 1~2 个关键的即可。
5. **外链不用加粗包围**：外部链接用标准 Markdown `[文字](地址)` 语法，
   不要用 `**…**` 把链接或整句加粗（容易破坏行内排版）。
...
- [ ] 简介 / 一句话定位正式、少技术细节：说清「是什么、解决什么问题」，别堆术语。
- [ ] 项目导向，没写成流程说明。
- [ ] 不暴露个人 / 环境细节（邮箱、本机路径、私有配置等）。
- [ ] 没罗列堆砌文件名、路径。
- [ ] 外部链接用 `[文字](地址)`，没被 `**…**` 加粗。
- [ ] 简介在预览之前，2~4 句讲清价值。
